...
export function NavigationBar({ providedFiles }) {
  ...
  return (
    <div className="bg-wash dark:bg-card-dark border-border dark:border-border-dark relative z-10 flex items-center justify-between rounded-t-lg border-b text-lg">
            ...
            <div className="relative overflow-hidden">
              ...
              <Listbox.Button as={Fragment}>
                {({ open }) => (
                  ...
                  <button className={cn('absolute left-[2px] top-0', !showDropdown && 'invisible')}>
                    <span
                      className={cn(
                        'text-link dark:text-link-dark border-link dark:border-link-dark text-md -mb-px mt-px flex h-full items-center truncate border-b px-1 py-2 leading-tight'
                      )}
                      style={{ maxWidth: '160px' }}
                    >
                      {getFileName(activeFile)}
                      {isMultiFile && <span className="ml-2">{open ? 'up' : 'down'}</span>}
                    </span>
                  </button>
                )}
              </Listbox.Button>
      ...
      <div className="flex items-center justify-end px-3 text-right" translate="yes">
